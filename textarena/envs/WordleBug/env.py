...
class WordleBugEnv(ta.Env):
    ...
    def _load_word_list(self, hardcore: bool = False) -> None:
        ...
        self.word_list = [
            ...
            if pos_tag([word])[0][1] in ["NN"]
            and len(word) == self.word_length
            and self._check_word(word.lower())
        ]

    def reset(self, num_players: int = 1, seed: Optional[int] = None):
        self.state = ta.SinglePlayerState(num_players=num_players, seed=seed)
        secret_word = random.choice(self.word_list).lower()
        game_state = {"secret_word": secret_word, "guess_history": [], "word_length": self.word_length, "num_guesses": self.num_guesses}
